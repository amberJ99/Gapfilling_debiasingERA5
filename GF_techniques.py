# -*- coding: utf-8 -*-
"""
Created on Fri Jan 12 12:03:35 2024
...
def determine_LP(begingap, endgap, seasonal_variation, positioning):
    """
    Determines the datetime values of the learning period.
    ...
    seasonal_variation : integer
        Size of the learning period expressed in number of days. This amount of days will be taken
        before or after the gap, or a combination of both.
    positioning : string
...
def GF_debmodelReg(df, name_gapped, name_model, seasonal_variation = 30, time_variation=0, positioning='both', degree=1, plot=False):
    ...
    seasonal_variation : integer, optional
        Size of the learning period expressed in number of days. This amount of days will be taken
        before or after the gap, or a combination of both (depending on the positioning). 
        The default is 30.
    ...
    degree : integer, optional
        Degree of the regression. The default is 1.
    plot : boolean, optional
        If plot==True, a plot is made with the observations and model data within the time window, inclusive the calculated regression. 
...
def GF_debmodelTvar(df, name_gapped, name_model, seasonal_variation = 30, time_variation=0, positioning='both'):
    ...
    The weights are determined based on similarities in temperature variation between the days in the learning period and the day of the filled missing value.
    ...
    for i, datesLP in enumerate(LP):
        ...
        for daygap in np.unique(datesgap.date):
            ...
            # Before next step: delete all NaN rows!
            # --> Not needed because if weight is NaN, also wtimesb will be NaN. 
            #     As long as Bias is never NaN, weight and wtimesb are always simultaneously NaN (--> both are not taken into account when taking .sum())
