# -*- coding: utf-8 -*-
"""
Created on Fri Jan 12 12:08:37 2024

@author: ambjacob
"""

from GF_techniques import *
...
def makegap(df, name_fulldata, lengthhour, starttime):
    ...
    df.loc[(df.index >= DateB) & (df.index < DateE),name_fulldata + '_gap']=float('nan') # of None kan ook
...
def calculate_error(df, name_fulldata, name_gapped, listerror, listtechnique):
    """
    Calculates the errors for the techniques.
    Example: 
...
def Perform_techniques(df, name_fulldata, name_model, gap_datetime, gap_length, dictionarytechniques, listerrors, plot=False, check=False):
    ...
    dictionarytechniques : dictionary
        ...
        If for a certain key the dictionary consists of a list, the values of the list are used as arguments of the function (besides the dataframe and the name of the columns as argument). 
    ...
    plot : boolean, optional
        If plot==True: the original complete data + ERA5 data + filled data is plotted for the gap. The default is False.
    check : boolean, optional
        If check == True everytime a gap filling technique is performed this is printed on the screen. The default is False.
    ...
    if plot==True:
        ...
        # print('Dataframe after performing all GF techniques:')
        # print (dfgapsl[[name_fulldata, name_model, 'P_linint', 'P_fillmodel','P_debmodelReg','P_debmodelMeanbias', 'P_debmodelTvar']].to_string())
...
def Test_techniques_differentgapdates(df, name_fulldata, name_model, gap_length, dictionarytechniques, par_slicedates, listerrors, repetitions, check=1):
    ...
    dictionarytechniques : dictionary
        Dictionary with the name of the different G-Ftechniques as key. 
        Possibilities: 'linint', 'fillmodel', 'debmodelReg', 'debmodelMeanbias', 'debmodelTvar'.
        If for a certain key the dictionary consists of a list, the values of the list are used as arguments of the
        function (besides the dataframe and the name of the columns as argument). If it is not a list, only
        the dataframe will be an argument for the function. 
    ...
    par_slicedates : integer
        The amount of days which will be sliced from the beginning and end of the
        dataset, so the gap will be placed far enough from the edges of the dataset according to the seasonalW. The most
        logical par_slicedates will be seasonalW/2, because the learning period will be placed symmetrical around the
        gap.
    listerrors : list of strings
    ...
    check : integer, optional
        After this amount of gaps made a text is printed on the screen with the number of
        repetition we are currently at. The default is 1.
    ...
    # Determine possible datetimes of the begin of the gap (so learning period can always be placed around/before/after gap)
    # IDEA: determine cut-off period based on the chosen length of the learning period + chosen technique (both, separate or left, right)
    alldates=df.index
...
def Test_techniques_differentgaplengths(df, name_fulldata, name_model, dictionarytechniques, par_slicedates, error, range_gaplengths, repetitions, check=1, plot=False):
    ...
    check : integer, optional
        ...
        repetition we are currently at. The default is 1.
    plot : boolean, optional
        If plot==True: a plot is made for the mean error in function of the gap_length for the different techniques. The default is False.
    ...
    # If plot==True: plot error ifv gap_length voor elke techniek
    if plot==True:
